...
//    child_tx_func.SetValid(resetn == RESETN_VALID);
    ...
    ILA_INFO << "before writing payload";
    {
      ...
      auto delayed_rd_ptr = Ite((wcnt > 7) ,             Ite(fifo_rd_ptr >= 3, fifo_rd_ptr - 3, BvConst(TXFIFO_BUFF_DEPTH, TXFIFO_BUFF_RD_PTR_BWID) - (BvConst(0x3, TXFIFO_BUFF_RD_PTR_BWID) - fifo_rd_ptr)), 
                            Ite((wcnt >= 0 & wcnt <= 7), Ite(fifo_rd_ptr >= 2, fifo_rd_ptr - 2, BvConst(TXFIFO_BUFF_DEPTH, TXFIFO_BUFF_RD_PTR_BWID) - (BvConst(0x2, TXFIFO_BUFF_RD_PTR_BWID) - fifo_rd_ptr)),
